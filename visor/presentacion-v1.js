(() => {
 ...
 function insert(){const bar=document.getElementById('viewer-toolbar');if(!bar)return;const link=document.createElement('a');link.id='download-presentation';link.href='AMPM-Local133-Presentacion-R43.pdf';link.download='AMPM-Local133-Presentacion.pdf';link.type='application/pdf';link.textContent='Descargar presentación PDF';link.title='9 páginas: áreas principales y plano arquitectónico';bar.append(link);const plan=document.getElementById('plan-close');if(plan){const copy=link.cloneNode(true);copy.id='download-presentation-plan';plan.before(copy)}observer.disconnect();}
