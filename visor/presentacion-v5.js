(() => {
 ...
 function insert(){const bar=document.getElementById('viewer-toolbar');if(!bar)return;const link=document.createElement('a');link.id='download-presentation';link.href='AMPM Layout RD - 136.42 m2 - Presentacion.pdf';link.download='AMPM Layout RD - 136.42 m2 - Presentacion.pdf';link.type='application/pdf';link.textContent='AMPM Layout RD · 136,42 m² · PDF';link.title='10 páginas: áreas principales y plano arquitectónico';bar.append(link);const plan=document.getElementById('plan-close');if(plan){const copy=link.cloneNode(true);copy.id='download-presentation-plan';plan.before(copy)}observer.disconnect();}
